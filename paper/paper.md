...
PyClustering library is available on PyPi and github repository. Since the first release on PyPi in 2014, it has been downloaded more than 138.000 times. Quality of the library is supported by static and dynamic analyzers such as cppcheck, scan-build, valgrid [@Nethercote2007], including compilers gcc, clang, VS2015. Code coverage is more then 93% by unit and integration tests (total amount of tests is than 2.200 tests). Each commit to repository triggers build, analysis and testing processes on CI services such as travis-ci, appveyor. PyClustering provides fully documented code for each library version including examples, math and algorithms description, installation instructions. API documentation is generated by doxygen without any warnings and notes to ensure completeness.
