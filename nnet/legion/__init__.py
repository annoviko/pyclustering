...
class legion_network(network, network_interface):
    ...
    def allocate_sync_ensembles(self, tolerance = 0.1):
        ...
        descriptors = [] * len(self._dyn_exc);
        
        # Check from the end for obtaining result
        for index_dyn in range(0, len(self._dyn_exc[0]), 1):
            time_stop_simulation = len(self._dyn_exc) - 1;
            active_state = False;
            
            if (self._dyn_exc[time_stop_simulation][index_dyn] > threshold):
                active_state = True;
            ...
            if (active_state is True):
                while ( (self._dyn_exc[time_stop_simulation][index_dyn] > threshold) and (time_stop_simulation > 0) ):
            ...
            for t in range(time_stop_simulation, 0, -1):
                if ( (self._dyn_exc[t][index_dyn] > 0) and (active_state is False) ):
                    desc[0] = t;
                    active_state = True;
                elif ( (self._dyn_exc[t][index_dyn] < 0) and (active_state is True) ):
                    desc[1] = t;
                    active_state = False;
